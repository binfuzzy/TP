...
public class VentanaPrincipal extends JFrame implements ObservadorSimuladorTrafico {
		...
		int returnVal = this.fc.showSaveDialog(null);
		 ...
		 if (returnVal == JFileChooser.APPROVE_OPTION) {
			 ...
			 try {
				 ...
				 JOptionPane.showMessageDialog(null, "Error al guardar los eventos", "Ha habido un error", JOptionPane.WARNING_MESSAGE);
			 }
		 }
	}
		...
		int returnVal = this.fc.showSaveDialog(null);
		 ...
		 if (returnVal == JFileChooser.APPROVE_OPTION) {
			 ...
			 try {
				 ...
				 JOptionPane.showMessageDialog(null, "Error al guardar los informes", "Ha habido un error", JOptionPane.WARNING_MESSAGE);
	...
	public int getSteps() {
		...
		steps.setValue(0);
		JOptionPane.showMessageDialog(null, steps, "Cuantos pasos quieres ejecutar?", JOptionPane.QUESTION_MESSAGE);
	...
	/*public void ejecuta(int i) throws Exception, ErrorDeSimulacion, BadSimulationObject, IOException {
		if (worker == null){
	...
	private void sleepThread() {
		try {
			Thread.sleep((Integer) this.toolbar.getDelay().getValue());
