...
public class Board {
	...
	public Cell getCell(Position pos) {
		...
		return this._board[pos.getRow()][pos.getCol()];
	...
	public boolean isFull() {
		...
		for(int i = 0; i < this._boardSize && fullBoard; i++) {
			for(int j = 0; j < this._boardSize && fullBoard; j++) {
	...
	public boolean canMerge(GameRules gameRules) {
		
		return gameRules.checkMoves(this);
	...
	/*Guarda el tablero en el fichero que tiene abierto el buffer*/

	public void save(BufferedWriter out) throws IOException {
		int i, j;
		for(i = 0; i < this._boardSize; i++) {
			for(j = 0; j < this._boardSize; j++) {
				out.write(this._board[i][j].getValue() + " ");
	...
	/*Carga la parte del tablero correspondiente al fichero abierto por el buffer*/
	
	public void loadBoard(BufferedReader buffer) throws IOException, ExecutionException {
		...
		for (int i = 0; i < size; i++) {
			for (int j = 0; j < size; j++) {
				...
				try {
					...
					throw new ExecutionException("Cells were corrupted (not numbers)\n");
				}
				
				this._board[i][j] = new Cell(bufferedBoard[i][j]);
			}
			
			line = buffer.readLine();
			vector = line.split("\\s+");
			if(vector.length != size && i < size-1)
				throw new ExecutionException("This is not a squared board\n");
